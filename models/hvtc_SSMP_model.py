...
class hvtcSSMPModel(BaseModel):
    '''
    纯净 hvtc + cross-stitch
    默认判别器 ( 3-layer 的 patchGAN )
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            parser.add_argument('--no_vgg_loss', action='store_false', dest='vgg_loss')
            parser.add_argument('--no_gan_feat_loss', action='store_false', dest='gan_feat_loss')
            ...
            parser.set_defaults(batch_size=2, netD='multiscale', gan_feat_loss=True, vgg_loss=True)
        return parser
    ...
    def __init__(self, opt):
        ...
        self.tasks = ['tran', 'seg']
        self.stages = ['conv3', 'conv4', 'resblocks']
        ...
        if self.isTrain:
            self.model_names = ['Shared_Encoder', 'Tran_Encoder', 'Seg_Encoder', 'TranHead', 'SegHead', 'D', 'Cross_Stitch']
        else:  # during test time, only load G
            self.model_names = ['Shared_Encoder', 'Tran_Encoder', 'Seg_Encoder', 'TranHead', 'SegHead', 'Cross_Stitch']
        ...
        (self.netShared_Encoder, 
         self.netTran_Encoder, 
         self.netSeg_Encoder, 
         self.netTranHead, 
         self.netSegHead) = mtl_networks.define_sShareNet(
        ...
        self.channel = {'conv3': 512, 'conv4': 1024}
        self.netCross_Stitch = torch.nn.ModuleDict({
            stage: mtl_networks.CrossStitchUnit(self.tasks, self.channel[stage], opt.alpha, opt.beta) 
            for stage in self.stages
            if stage in self.channel
            })
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam([
                {'params': itertools.chain(self.netShared_Encoder.parameters(), self.netTran_Encoder.parameters(), 
                                           self.netSeg_Encoder.parameters(), self.netTranHead.parameters(), self.netSegHead.parameters()), 'lr': opt.lr},
    ...
    def forward(self):
        ...
        ### shared_Encoder forward
        self.shared_feat = self.netShared_Encoder(self.real_A)

        ### task_Encoder & cross_stitch forward
        x = {task: self.shared_feat for task in self.tasks}
        task_encoders = {'tran': self.netTran_Encoder,
                         'seg': self.netSeg_Encoder}

        ### forward task_Encoder
        for stage in self.stages:
            for task in self.tasks:
                x[task] = task_encoders[task].module.forward_stage(x[task], stage)
        ...
        self.fake_B = self.netTranHead(x['tran'])
        self.pred_map = self.netSegHead(x['seg'])
