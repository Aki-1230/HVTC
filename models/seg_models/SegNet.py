...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.encode1(x) # channel 3 -> 64, size 256 -> 256
        x, id1 = F.max_pool2d_with_indices(x, kernel_size=2, stride=2, return_indices=True) # channel 64 -> 64, size 256 -> 128
        idx.append(id1)

        x = self.encode2(x) # channel 64 -> 128, size 128 -> 128
        x, id2 = F.max_pool2d_with_indices(x, kernel_size=2, stride=2, return_indices=True) # channel 128 -> 128, size 128 -> 64
        idx.append(id2)

        x = self.encode3(x) # channel 128 -> 256, size 64 -> 64
        x, id3 = F.max_pool2d_with_indices(x, kernel_size=2, stride=2, return_indices=True) # channel 256 -> 256, size 64 -> 32
        idx.append(id3)
